...
public final class TestResources {
    ...
    public record TestImage(URL resource, int width, int height, int loopCount, TestFrame[] frames) {
        ...
        public TestImage(URL resource, int width, int height, int loopCount, int[] delays) {
            ...
            for (int i = 0; i < delays.length; i++) {
                this.frames[i] = new TestFrame(width, height, delays[i]);
            }
        }
    }

    public record TestFrame(int width, int height, int delay) {
    }
}
